...
def test_nlf32():
    ...
    with LinearModel(dll='nlf32') as m:
        ...
        assert m.dll_path != 'nlf32.dll'
        assert os.path.basename(m.dll_path) == 'nlf32.dll'
...
def test_nlf64():
    ...
    if IS_PYTHON_64BIT:
        ...
        with LinearModel(dll=dll) as m:
            assert str(m) == expect
            assert repr(m) == expect
            assert m.dll_path != 'nlf64.dll'
            assert os.path.basename(m.dll_path) == 'nlf64.dll'
    else:
        with pytest.raises(ValueError, match=r'64-bit DLL in 32-bit Python'):
            with LinearModel(dll=dll):
                pass


def test_dll_preserved_binary_operator():
    # the original DLL path that was chosen is preserved when a
    # binary operator is applied
