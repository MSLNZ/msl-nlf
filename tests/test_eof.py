...
from msl.nlf import ResidualType
...
def test_eof(abs_res, res_typ, method, expected):
    # The expected value comes from the Delphi GUI
    #
    # For MiniMax fitting, the GUI returns the maximum residual
    # whereas the DLL returns the error of fit (therefore we
    # calculate the expected value below)

    x = [77.6, 114.9, 141.1, 190.8, 239.9, 289.0, 332.8, 378.4,
    ...
    with LinearModel() as model:
        ...
        model.options(absolute_residuals=abs_res,
                      residual_type=res_typ, fit_method=method)
