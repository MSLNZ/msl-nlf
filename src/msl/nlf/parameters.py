...
class Parameter:
    ...
    def __init__(self, name: str, value: float, *, label: str | None = None) -> None:
        ...
        Args:
            name: The name of the parameter in the equation (e.g., `a1`).
            value: The value of the parameter.
            label: A custom label associated with the parameter. For example, if the
...
class InputParameter(Parameter):
    ...
    def __init__(self, name: str, value: float, *, constant: bool = False, label: str | None = None) -> None:
        ...
        Args:
            name: The name of the parameter in the equation (e.g., `a1`).
            value: The value of the parameter.
            constant: Whether the parameter is held constant (`True`) or allowed to vary
                (`False`) during the fitting process.
            label: A custom label associated with the parameter. For example, if the
                fit equation is `a1+a2*x`, you could assign a label of *intercept*
                to `a1` and *slope* to `a2`.
        """
    ...
    def constant(self) -> bool:
        ...
        Returns:
            Whether the parameter is held constant.
        """
...
class ResultParameter(Parameter):
    ...
    def __init__(self, name: str, value: float, uncert: float, *, label: str | None = None) -> None:
        ...
        Args:
            name: The name of the parameter in the equation (e.g., `a1`).
            value: The value of the parameter.
            uncert: The standard uncertainty of the parameter.
            label: A custom label associated with the parameter. For example, if the
                fit equation is `a1+a2*x`, you could assign a label of *intercept*
                to `a1` and *slope* to `a2`.
        """
...
class Parameters(Generic[T]):
    ...
    def labels(self) -> list[str | None]:
        """Returns the [label][msl.nlf.parameters.Parameter.label] of each parameter.

        Returns:
            A list of [label][msl.nlf.parameters.Parameter.label]s.
        """
        return [p.label for p in self]

    def names(self) -> list[str]:
        """Returns the [name][msl.nlf.parameters.Parameter.name] of each parameter.

        Returns:
            A list of [name][msl.nlf.parameters.Parameter.name]s.
        """
        return [p.name for p in self]

    def values(self) -> NDArray[np.float64]:
        """Returns the [value][msl.nlf.parameters.Parameter.value] of each parameter.

        Returns:
            A list of [value][msl.nlf.parameters.Parameter.value]s.
        """
        return np.array([p.value for p in self], dtype=float)


class InputParameters(Parameters[InputParameter]):
    """A collection of [InputParameter][msl.nlf.parameters.InputParameter]s for a fit model."""

    def __init__(self, parameters: Iterable[InputParameterType] | None = None) -> None:
        """A collection of [InputParameter][msl.nlf.parameters.InputParameter]s for a fit model.

        Args:
            parameters: An iterable of either [InputParameter][msl.nlf.parameters.InputParameter] instances
                or objects that can be used to create an [InputParameter][msl.nlf.parameters.InputParameter]
                instance. See [add][msl.nlf.parameters.InputParameters.add] for examples.
        """
        super().__init__()
        if parameters:
    ...
    def add(self, *args: Any, **kwargs: Any) -> InputParameter:  # noqa: ANN401
        """Add an [InputParameter][msl.nlf.parameters.InputParameter].

        An [InputParameter][msl.nlf.parameters.InputParameter] can be added using either positional or
        keyword arguments, but you cannot use both simultaneously. You can specify positional
        arguments by using one of four options:

        * InputParameter (a single argument must be an [InputParameter][msl.nlf.parameters.InputParameter])
        * name, value
        * name, value, constant
        * name, value, constant, label

        You could alternatively add an [InputParameter][msl.nlf.parameters.InputParameter] in the same way
        that you add items to a [dict][].
    ...
    def add_many(self, parameters: Iterable[InputParameterType]) -> None:
        """Add many [InputParameter][msl.nlf.parameters.InputParameter]s.

        Args:
            parameters: An iterable of either [InputParameter][msl.nlf.parameters.InputParameter] instances
                or objects that can be used to create an [InputParameter][msl.nlf.parameters.InputParameter] instance.
                See [add][msl.nlf.parameters.InputParameters.add] for more examples.
    ...
    def clear(self) -> None:
        """Remove all [InputParameter][msl.nlf.parameters.InputParameter]s from the collection."""
        self._map.clear()

    def constants(self) -> NDArray[np.bool_]:
        """Returns the [constant][msl.nlf.parameters.InputParameter.constant] of each parameter.

        Returns:
            A list of [constant][msl.nlf.parameters.InputParameter.constant]s.
        """
        return np.array([p.constant for p in self], dtype=bool)

    def pop(self, name_or_label: str) -> InputParameter:
        """Pop an [InputParameter][msl.nlf.parameters.InputParameter] from the collection.

        This will remove the [InputParameter][msl.nlf.parameters.InputParameter] from the collection and return it.

        Args:
            name_or_label: The [name][msl.nlf.parameters.Parameter.name] or
                [label][msl.nlf.parameters.Parameter.label] of an [InputParameter][msl.nlf.parameters.InputParameter]
    ...
    def update(self, name_or_label: str, **attribs: Any) -> None:  # noqa: ANN401
        """Update the attributes of an [InputParameter][msl.nlf.parameters.InputParameter].
...
class ResultParameters(Parameters[ResultParameter]):
    """A collection of [ResultParameter][msl.nlf.parameters.ResultParameter]s from a fit model."""

    def __init__(self, result: dict[str, Any], params: InputParameters) -> None:
        """A collection of [ResultParameter][msl.nlf.parameters.ResultParameter]s from a fit model.

        Args:
            result: The result from a fit model.
    ...
    def uncerts(self) -> NDArray[np.float64]:
        """Returns the [uncert][msl.nlf.parameters.ResultParameter.uncert] of each parameter.

        Returns:
            A list of [uncert][msl.nlf.parameters.ResultParameter.uncert]s.
        """
