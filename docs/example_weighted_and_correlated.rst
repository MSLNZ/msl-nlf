.. _nlf-weighted-correlated:

===========================
2D, Weighted and Correlated
===========================
This example uses a :class:`~msl.nlf.model.Model` with the equation specified
as a string. The independent variable (stimulus) data is two-dimensional (i.e.,
contains :math:`x_1` and :math:`x_2` variables). There are uncertainties in both
:math:`x` and :math:`y` variables and the :math:`y-y` correlation coefficient is
0.5, the :math:`x_1-x_1` correlation coefficient is 0.8 and the
:meth:`~msl.nlf.model.Model.set_correlation` method is called to set the
correlation matrices in the model.
...
.. invisible-code-block: python
    ...
    import os
    with open('docs/_static/example_weighted_correlated_input.txt', mode='wt') as fp:
        out = []
        lines = str(model_input).splitlines()
        for line in lines:
            if 'path=' in line:
                _, path = line.split('=')
                out.append(fr"          path='...\{os.path.basename(path)}")
            else:
                out.append(line)
        fp.write('\n'.join(out))

.. literalinclude:: _static/example_weighted_correlated_input.txt
...
.. invisible-code-block: python
    ...
    with open('docs/_static/example_weighted_correlated_result.txt', mode='wt') as fp:
        fp.write(str(result))

.. literalinclude:: _static/example_weighted_correlated_result.txt
